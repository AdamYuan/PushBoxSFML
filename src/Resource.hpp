...
constexpr const char *kShadowBlurGlsl = R"(
uniform sampler2D uTexture;
uniform vec2 uDirection, uResolution;
...
float blur13(sampler2D image, vec2 uv, vec2 resolution, vec2 direction) {
	...
	color += texture2D(image, uv).a * 0.1964825501511404;
	color += texture2D(image, uv + (off1 / resolution)).a * 0.2969069646728344;
	color += texture2D(image, uv - (off1 / resolution)).a * 0.2969069646728344;
	color += texture2D(image, uv + (off2 / resolution)).a * 0.09447039785044732;
	color += texture2D(image, uv - (off2 / resolution)).a * 0.09447039785044732;
	color += texture2D(image, uv + (off3 / resolution)).a * 0.010381362401148057;
	color += texture2D(image, uv - (off3 / resolution)).a * 0.010381362401148057;
	return color;
}

void main() {
	float val = blur13(uTexture, gl_TexCoord[0].xy, uResolution, uDirection);
	gl_FragColor = vec4(0.0, 0.0, 0.0, val);
}
)";

constexpr const char *kShadowFinalGlsl = R"(
uniform sampler2D uTexture;
void main() {
	float val = texture2D(uTexture, gl_TexCoord[0].xy).a;
	gl_FragColor = vec4( 0, 0, 0, val * 2.0 );
}
)";
